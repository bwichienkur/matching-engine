...
{
    public class PostInstitutionActionFilter : ActionFilterBase
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            var postalCode = requestValues["PostalCode"];
            if (!String.IsNullOrEmpty(postalCode))
            {
                bool isValidUSPostalCode = false;
                postalCode = postalCode.Trim();
                if (Utilities.InputValidation.IsValidUSPostalCode(postalCode))
                {
                    if (locations.GetAllUSPostalCodes().Where(pc => pc.PostalCodeString == postalCode).FirstOrDefault() != null)
                    {
                        isValidUSPostalCode = true;
                    }
                }

                if (!isValidUSPostalCode)
                {
