...
{
    public class SubjectFilter : ActionFilterBase
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            var subjectIds = requestValues["subjectIds"];
            PerformValidation(subjectIds, actionContext, ref responseContent);
        ...
        protected virtual void PerformValidation(string subjectIds, HttpActionContext actionContext, ref VendorResponseBase responseContent)
        {
            if (!string.IsNullOrEmpty(subjectIds))
            {
                ...
                if (subjectIdsArray != null)
                {
                    if (subjectIdsArray.Length > 0)
                    {
                        ...
                        if (isArrayInputNotNumeric(subjectIdsArray))
                        {
                            if (!isLeadSave)
                            {
                                responseContent.MessageCodes.Add(InputValidation.MessageCodes.SubCategoryIdsNotNumeric);
                            }
                            else
                            {
                                ContactRequest contactRequest = (ContactRequest)actionContext.ActionArguments["contactRequest"];
                                contactRequest.SubjectIds = new List<int>();
                            }
                        }
                    }
                }
