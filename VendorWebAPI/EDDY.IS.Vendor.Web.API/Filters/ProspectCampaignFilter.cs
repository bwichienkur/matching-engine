...
{
    public class ProspectCampaignFilter : ActionFilterBase
    {
            ...
            if (requestValues != null)
            {
                ...
                if (!String.IsNullOrEmpty(apikeyParam))
                {
                    ...
                    if (Guid.TryParse(apikeyParam, out apiKey))
                    {
                        ...
                        {
                            if (!vendorCampaign.IsProspect)
                            {
                                VendorResponseMessage message = vendorResponseMessages.GetVendorResponseMessageByMessageCode(InputValidation.MessageCodes.FeatureNotConfiguredProspecting);
                                if (message != null)
                                {
