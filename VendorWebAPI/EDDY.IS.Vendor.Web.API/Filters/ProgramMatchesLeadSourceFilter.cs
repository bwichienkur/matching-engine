...
{
    public class ProgramMatchesLeadSourceFilter : ActionFilterBase
    {
            ...
            if (vendorCampaign != null)
            {
                if (vendorCampaign.ApplicationId != 27)
                {
                    if (vendorCampaign.IsSourceRequired)
                    {
                        ...
                        if (!String.IsNullOrEmpty(ConfigurationManager.AppSettings.Get("RequireProgramMatchesContactSource")))
                        {
                            isRequired = Convert.ToBoolean(ConfigurationManager.AppSettings.Get("RequireProgramMatchesContactSource"));
                        }
