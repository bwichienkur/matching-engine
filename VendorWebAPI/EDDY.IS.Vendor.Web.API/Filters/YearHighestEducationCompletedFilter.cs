...
{
    public class YearHighestEducationCompletedFilter : ActionFilterBase
    {
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            var yearHighestEducationCompleted = requestValues["YearHighestEducationCompleted"];
            PerformValidation(yearHighestEducationCompleted, ref responseContent);
        ...
        private void PerformValidation(string yearHighestEducationCompleted, ref VendorResponseBase vendorResponseBase)
        {
            int yearHighestEducationCompletedInt;

            if (!string.IsNullOrEmpty(yearHighestEducationCompleted))
            {
                if (this.isInputNotNumeric(yearHighestEducationCompleted))
                {
                    vendorResponseBase.MessageCodes.Add(InputValidation.MessageCodes.YearHighestEducationCompletedIsNotNumeric);
                }
                else if (int.TryParse(yearHighestEducationCompleted, out yearHighestEducationCompletedInt))
                {
                    if (yearHighestEducationCompletedInt > DateTime.Now.Year || yearHighestEducationCompletedInt < 1940)
                    {
                        vendorResponseBase.MessageCodes.Add(InputValidation.MessageCodes.YearHighestEducationCompletedInvalid);
                    }
                }
            }
        }
