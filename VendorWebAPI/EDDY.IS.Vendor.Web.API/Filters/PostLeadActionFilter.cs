...
{
    public class PostLeadActionFilter : ActionFilterBase
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (string.IsNullOrEmpty(leadIdToken) && vendorCampaign?.ApplicationId != _emsApplicationId && vendorCampaign?.ChannelId != 13)
            {
            ...
            if (isStateValid && isCountryValid && !string.IsNullOrEmpty(postalCode))
            {
                if (country.ToLower() == "us")
                {
                    ...
                    else if (!locations.PostalCodeExistsWithinState(postalCode, state, NorthAmericanCountry.UnitedStates))
                    {
