...
{
    public class HostAndPostCampaignFilter : ActionFilterBase
    {
            ...
            if (requestValues != null)
            {
                ...
                if (!String.IsNullOrEmpty(apikeyParam))
                {
                    ...
                    if (Guid.TryParse(apikeyParam, out apiKey))
                    {
                        ...
                        {
                            if (!vendorCampaign.IsHostAndPost)
                            {
                                VendorResponseMessage message = vendorResponseMessages.GetVendorResponseMessageByMessageCode(InputValidation.MessageCodes.FeatureNotConfiguredHP);
                                if (message != null)
                                {
