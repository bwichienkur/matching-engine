...
{
    public class AgeFilter : ActionFilterBase
    {
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            var age = requestValues["Age"];
            PerformValidation(age, ref responseContent);
        ...
        private void PerformValidation(string age, ref VendorResponseBase responseContent)
        {
            int ageInt;

            if (!string.IsNullOrEmpty(age))
            {
                if (this.isInputNotNumeric(age))
                {
                    responseContent.MessageCodes.Add(InputValidation.MessageCodes.AgeNotNumeric);
                }
                else if (int.TryParse(age, out ageInt))
                {
                    if (ageInt > 100)
                    {
                        responseContent.MessageCodes.Add(InputValidation.MessageCodes.AgeLimitExceeded);
                    }
                }
            }
        }
